...
func main() {
	...
	// ----- Test Telebirr SMS -----
	telebirrSamples := []string{
		`Dear FTSUMBRHAN You have received ETB 5.00 from Lidia Mulugeta(2519****5740) on 14/10/2025 18:45:12. Your transaction number is CJE1GZUZCN. Your current E-Money Account balance is ETB 192.59. Thank you for using telebirr Ethio telecom.`,
	}

	fmt.Println("\n===== Telebirr SMS Parser Test =====")
	for i, sms := range telebirrSamples {
		fmt.Printf("\n--- Sample %d ---\n", i+1)
		parsed, err := utils.ParseBankMulti(sms)
		if err != nil {
			fmt.Println("Error parsing SMS:", err)
			continue
		}
